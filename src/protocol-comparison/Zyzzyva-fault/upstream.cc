...
void UpstreamNode::recv_input()
{
    ...
    else if (msg_type == MessageType::COMMIT)
    {
        CommitMsgHelper commit_msg_helper(msg.content);
        /* verify the signature */
        if (crypto_sign_verify_detached(commit_msg_helper.tag,
                                        reinterpret_cast<uint8_t *>(commit_msg_helper.job_id),
                                        CommitMsgHelper::size - crypto_sign_BYTES,
                                        downstream_pub_keys_[0].data()) != 0)
        {
            throw std::runtime_error("Invalid signature in commit");
        }
        if (memcmp(commit_msg_helper.history_hash, history_hash_, crypto_generichash_BYTES))
        {
            dumpMemory(history_hash_, crypto_generichash_BYTES);
            dumpMemory(commit_msg_helper.history_hash, crypto_generichash_BYTES);
            throw std::runtime_error("History hash mismatch in commit");
        }

        /* verify the matching tags */
        for (int i = 0; i < 2 * FAULTY_NODES + 1; i++)
        {
            uint64_t upstream_id = commit_msg_helper.matching_replica_ids[i];
            uint8_t *upstream_tag = commit_msg_helper.matching_tags + i * crypto_sign_BYTES;
            if (crypto_sign_verify_detached(upstream_tag,
                                            reinterpret_cast<uint8_t *>(commit_msg_helper.job_id),
                                            sizeof(*commit_msg_helper.job_id) + crypto_generichash_BYTES * 2,
                                            upstream_pub_keys_[upstream_id].data()) != 0)
            {
                std::cerr << i << " " << upstream_id << std::endl;
                dumpMemory(upstream_tag, crypto_sign_BYTES);
                throw std::runtime_error("Invalid matching tag in commit");
            }
        }

        /* send the local commit message to downstream */
        Message local_commit_msg(MessageType::LOCAL_COMMIT);
        LocalCommitMsgHelper local_commit_msg_helper(local_commit_msg.content);
        *local_commit_msg_helper.job_id = *commit_msg_helper.job_id;
        memcpy(local_commit_msg_helper.history_hash, commit_msg_helper.history_hash, crypto_generichash_BYTES);
        memcpy(local_commit_msg_helper.reply_hash, commit_msg_helper.reply_hash, crypto_generichash_BYTES);

        /* sign the local commit */
        unsigned long long siglen;
        crypto_sign_detached(local_commit_msg_helper.tag,
                             &siglen,
        ...
        auto sent = sendto(socket_fd_,
                           &local_commit_msg,
                           LocalCommitMsgHelper::size + sizeof(MessageType),
                           0,
                           (struct sockaddr *)&dest_addr,
                           sizeof(dest_addr));
        if (static_cast<uint64_t>(sent) != LocalCommitMsgHelper::size + sizeof(MessageType))
        {
            throw std::runtime_error("Error! " + std::to_string(sent) + " bytes sent instead of " + std::to_string(LocalCommitMsgHelper::size + sizeof(MessageType)));
        }
        profiler.mark_end();
    }
    else
...
void UpstreamNode::exec_task(Message &input_msg)
{
    ...
    *result_msg_helper.job_id = current_job_id_;
    ...
    // dumpMemory(result_msg_helper.meta_tag, crypto_sign_BYTES);
}
