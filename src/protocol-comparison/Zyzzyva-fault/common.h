...
public:
    ...
    uint8_t *history_hash;
    uint8_t *reply_hash;
    uint64_t *matching_replica_ids; /* 2f+1 ids */
    uint8_t *matching_tags;         /* 2f+1 tags */

    CommitMsgHelper(uint8_t *buf) : tag(buf),
                                    job_id(reinterpret_cast<uint64_t *>(buf + crypto_sign_BYTES)),
                                    history_hash(reinterpret_cast<uint8_t *>(job_id + 1)),
                                    reply_hash(history_hash + crypto_generichash_BYTES),
                                    matching_replica_ids(reinterpret_cast<uint64_t *>(reply_hash + crypto_generichash_BYTES)),
                                    matching_tags(reply_hash + crypto_generichash_BYTES + (2 * FAULTY_NODES + 1) * sizeof(*matching_replica_ids)) {}
    const static size_t size = crypto_sign_BYTES + sizeof(uint64_t) + crypto_generichash_BYTES + crypto_generichash_BYTES + (2 * FAULTY_NODES + 1) * (sizeof(*matching_replica_ids) + crypto_sign_BYTES);
};

class LocalCommitMsgHelper
{
public:
    uint8_t *tag;
    uint64_t *job_id;
    ...
    LocalCommitMsgHelper(uint8_t *buf) : tag(buf),
                                         job_id(reinterpret_cast<uint64_t *>(buf + crypto_sign_BYTES)),
                                         history_hash(reinterpret_cast<uint8_t *>(job_id + 1)),
                                         reply_hash(history_hash + crypto_generichash_BYTES) {}
    const static size_t size = crypto_sign_BYTES + sizeof(uint64_t) + crypto_generichash_BYTES + crypto_generichash_BYTES;
};
