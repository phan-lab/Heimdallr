...
Node::Node(int port, NodeType node_type, uint64_t id, int num_nodes)
    : my_id_(id), num_nodes_(num_nodes),
      current_job_id(0),
      PoC_combined_(MessageType::POC),
      PoC_combined_helper_(PoC_combined_.content)
{
    profiler.reset_and_output();
    if (num_nodes_ != FAULTY_NODES + 1)
    {
        throw std::runtime_error("Incorrect f and #nodes, got " +
                                 std::to_string(FAULTY_NODES) + " and " +
...
uint64_t Node::getCurrentTimestamp()
{
