...
for ((i=0; i<num_servers; i++)); do
    port=$((base_port + i))
    taskset -c $(( (cpu_max_id - i % NCPU + NCPU) % NCPU )) ./upstream $port $i $num_clients_ > logs_$1/servers/server_${i}.log 2>logs_$1/servers/server_${i}.stats &
    echo "Started server $i on port $port"
done

echo "Started $num_servers servers, each serving $num_clients_ clients"